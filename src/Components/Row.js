...
import { onePlayerContext } from "../App";

const baseUrl = "https://image.tmdb.org/t/p/original/"; //posterpath

function Row({ title, fetchUrl, isLargeRow, rowNo }) {
  const one = useContext(onePlayerContext);
  ...
  const handleClick = async (movie) => {
    ...
    one.onePlayerDispatch({ type: "chechRowSync", value: rowNo });
  };
  ...
  return (
    <div className="row">
      ...
      {one.onePlayerState === rowNo && trailerUrl && (
